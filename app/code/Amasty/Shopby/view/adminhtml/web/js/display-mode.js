define([
    ...
    AmastyFormElementDependenceController = Class.create(FormElementDependenceController,
        {
            initialize: function ($super, elementsMap, groupValues, fieldsets, config) {
            ...
            onSelectChanged: function (e, index, values) {
                ...
                if (this.isNotEmpty(values)) {
                    ...
                    var group = this.getGroup(Element);

                    if (this.groupGen != group) {
                        this.changeDisplayOptions(index, values, group);
                    }
                    var displayMode = _.findKey(values.dependecies[group], function (value) {
                        return $(Element).value === value;
                    }.bind(this));
                    if (!this.isEditable(Element) || this.isEnabledTypes($(Element).value)) {
            ...
            onDisplayModeChanged: function (e, index, values) {
                ...
                if ((this.isEditable(index) &&
                        ...
                        _.has(values.dependecies[group], $(Element).value)
                    ) || this.isEnabledTypes($(index).value) || this.groupGen != 'price') {

                    $(index).setValue(values.dependecies[group][$(Element).value]);
